 ...
 */
function parseLogLines(output: string): Commit[] {
  if (!output) return [];
...
async function run(): Promise<void> {
  ...
  try {
    openPRs = execSync(
      'gh pr list --repo RockaRhymeLLC/CC4Me --author @me --state open --json number,title --jq \'.[] | "#\\(.number): \\(.title)"\'',
      {
        cwd: getProjectDir(),
        encoding: 'utf8',
        timeout: 15_000,
  ...
  if (upstreamCommits.length > 0) {
    parts.push('', `**${upstreamCommits.length} new upstream commit${upstreamCommits.length === 1 ? '' : 's'}** (not in your fork):`);
    ...
    parts.push('', 'Action: Review these and merge upstream changes into your fork:', '  `git merge upstream/main` (or cherry-pick specific commits)');
  }

  if (forkCommits.length > 0) {
    parts.push('', `**${forkCommits.length} fork-only commit${forkCommits.length === 1 ? '' : 's'}** (not in upstream):`);
    ...
    parts.push('', 'Review these for potential upstream contributions. Use `/upstream prepare` for any worth contributing.');
  }

  if (openPRs) {
    parts.push('', '**Open PRs against upstream:**');
  ...
  if (upstreamCommits.length === 0 && forkCommits.length > 0) {
    parts.push('', 'Your fork is ahead of upstream but not behind — no merge needed, just review fork-only commits for upstreaming.');
  }
