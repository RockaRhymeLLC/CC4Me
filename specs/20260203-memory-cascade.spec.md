...
9. **Consolidation runs as Claude**: The nightly job could inject a prompt that triggers the agent to do the review, rather than the daemon doing mechanical summarization. This lets the agent apply judgment about what's worth remembering.
...
Proposed: `0 5 * * *` (5am daily) — runs while the user is asleep, results ready for morning session. Open to other times.
