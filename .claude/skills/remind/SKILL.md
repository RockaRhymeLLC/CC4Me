...
Set reminders that fire at a specific date/time via Telegram, even when BMO isn't in an active session. Uses launchd one-shot jobs that self-clean after delivery.
...
2. **Plist**: `~/Library/LaunchAgents/com.cc4me.reminder.<id>.plist` — launchd job that fires at the scheduled time
...
4. **Get chat ID**: Use Dave's Telegram chat ID from memory (`7629737488`)
5. **Create the script**:
...
# Clean up
launchctl bootout gui/$(id -u) ~/Library/LaunchAgents/com.cc4me.reminder.ID.plist 2>/dev/null
rm -f ~/Library/LaunchAgents/com.cc4me.reminder.ID.plist
rm -f "$0"
```
...
<dict>
    <key>Label</key>
    <string>com.cc4me.reminder.ID</string>
    <key>ProgramArguments</key>
    <array>
        <string>/bin/bash</string>
        <string>$BASE_DIR/scripts/reminders/remind-ID.sh</string>
    </array>
    ...
    <string>$BASE_DIR/logs/reminder-ID.log</string>
...
8. **Load the plist**: `launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.cc4me.reminder.ID.plist`
9. **Verify**: `launchctl list | grep reminder.ID`
10. **Confirm**: Tell the user when the reminder will fire

### Listing Reminders

1. Glob for `~/Library/LaunchAgents/com.cc4me.reminder.*.plist`
...
1. Unload: `launchctl bootout gui/$(id -u) ~/Library/LaunchAgents/com.cc4me.reminder.ID.plist`
2. Delete plist: `rm ~/Library/LaunchAgents/com.cc4me.reminder.ID.plist`
...
```
Reminder set!
- Message: "Time to leave for Grant's game"
...
[20260131-1030] Sat Jan 31 at 10:30 AM
  "Time to leave for Grant's game"

[20260205-0900] Wed Feb 5 at 9:00 AM
  "Follow up with Will on CC4Me setup"
```
...
| launchd plists | `~/Library/LaunchAgents/com.cc4me.reminder.<id>.plist` |
| Logs | `logs/reminder-<id>.log` |

## Notes

- Reminders fire even when BMO isn't in an active Claude Code session
...
- Chat ID defaults to the user's (from `memory.md`). Override by specifying a different recipient in the message.
- Always use `telegram-send.sh` for reminders (not the transcript watcher) since reminders fire outside sessions
