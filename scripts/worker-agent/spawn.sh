...
# Usage: spawn.sh <name> <project_dir> <profile> <mission>
...
if [ -z "$NAME" ] || [ -z "$PROJECT_DIR" ] || [ -z "$MISSION" ]; then
  echo "Usage: spawn.sh <name> <project_dir> <profile> <mission>"
...
# Resolve the parent tmux session from config or environment
SCRIPT_DIR="$(cd "$(dirname "$0")/../.." && pwd)"
TMUX_SESSION="${CC4ME_TMUX_SESSION:-}"
if [ -z "$TMUX_SESSION" ] && command -v python3 >/dev/null; then
  TMUX_SESSION=$(python3 -c "
import yaml, sys
try:
    with open('$SCRIPT_DIR/cc4me.config.yaml') as f:
        print(yaml.safe_load(f).get('tmux', {}).get('session', ''))
except: pass
" 2>/dev/null)
fi
# Fallback: use current tmux session or 'main'
if [ -z "$TMUX_SESSION" ]; then
  TMUX_SESSION="${TMUX_PANE:+$(tmux display-message -p '#S' 2>/dev/null)}"
...
When you finish or get stuck, signal the daemon:
...
- Signal the parent agent when done or stuck
...
KICK_OFF="Read CLAUDE.local.md for your mission brief and complete the task described there. Update .worker/progress.md as you go. Signal the parent agent when done."
