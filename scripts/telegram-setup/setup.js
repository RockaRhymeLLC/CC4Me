...
async function main() {
  ...
  if (!phoneNumber) {
    console.log('Tip: You can pass your phone number as an argument:');
    console.log('  node setup.js "+1-555-123-4567"\n');
  }
  ...
  try {
    ...
    if (loginButton) {
      console.log('\nPlease log in with your phone number' + (phoneNumber ? ': ' + phoneNumber : ''));
    ...
    console.log('   1. Display name (e.g., "My Assistant")');
    console.log('   2. Username (must end in "bot", e.g., "my_assistant_bot")\n');
    ...
    if (token) {
      ...
      console.log('Could not automatically find token.');
    ...
    if (chatId) {
      ...
      console.log('Could not find chat ID automatically.');
    }
    ...
    console.log('\nStore these securely in your Keychain:');
    console.log(`  security add-generic-password -a "assistant" -s "credential-telegram-bot" -w "${token}" -U`);
    if (chatId) {
      console.log('\nAdd your chat ID to .claude/state/safe-senders.json');
    }
