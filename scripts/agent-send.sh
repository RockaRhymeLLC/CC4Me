...
#
# Examples:
#   agent-send.sh r2d2 "Hello R2!"
#   agent-send.sh r2d2 "idle" status
#   agent-send.sh r2d2 "Telegram chunking" coordination
...
if [ -z "$PEER" ] || [ -z "$MESSAGE" ]; then
  ...
  echo ""
  echo "  peer     Peer agent name (e.g., r2d2)"
  echo "  message  Message text to send"
  echo "  type     Message type: text (default), status, coordination"
  echo ""
  echo "Examples:"
  echo "  agent-send.sh r2d2 'Hello R2!'"
  echo "  agent-send.sh r2d2 'idle' status"
  exit 1
fi
